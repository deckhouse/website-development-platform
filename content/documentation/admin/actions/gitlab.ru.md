...
{{< alert level="info" >}}
Для выполнения каждого действия необходимо наличие токена GitLab. `token` — токен пользователя, от имени которого выполняется действие. Токен передаётся через механизм учётных данных и используется для аутентификации при вызове GitLab API (HTTP-заголовок `Private-Token`).
{{< /alert >}}
