...
{{< alert level="info" >}}
Для выполнения действий, взаимодействующих с GitLab, необходимы учётные данные.

Для действий, использующих GitLab API, требуется токен пользователя. Для действий, работающих с Git-репозиторием по HTTPS (например, `CreateRepositoryFromTemplate`), используются имя пользователя и пароль (токен).
{{< /alert >}}
