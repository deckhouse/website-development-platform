---
title: Обзор
description: Конфигурация действий в Deckhouse Development Portal (DDP) — запросы, форма, обновления сущностей, безопасность и авторизация.
weight: 10
---

Действия — это механизм DDP (портала) для запуска операций во внешних инфраструктурных системах и сервисах. С их помощью можно, например, создавать:
...
- «Встроенное (BuiltIn)» — логика выполнения действия описана внутри портала. Для встроенных действий необходимо выбрать один из преднастроенных бэкендов.
- «Вебхук (Webhook)» — логика выполнения действия полностью настраивается пользователем.

#### Параметры перезапуска

Если выполнение действия завершается с ошибкой, портал может автоматически выполнить повторные попытки.
...
{{< alert level="info" >}}
При выборе формата «Form URL Encoded» тело запроса должно содержать только плоские пары «ключ — значение». Вложенные структуры и массивы в этом формате не поддерживаются.
{{< /alert >}}
...
Каждое действие отправляет HTTP-запрос на встроенный бэкенд или на URL вебхука бэкенда. Обычно запрос включает тело запроса (`body`), в котором описывается, что именно будет отправлено. Тело запроса задаётся в YAML.
...
Если включена опция «Обновление параметров сущности», портал применяет правила обновления и записывает значения в параметры сущности.
...
Например, при выполнении действия «Создание проекта в GitLab» в ответе (`response`) будет сохранена спецификация созданного проекта:
...
Если включена опция «Создание сущностей», портал автоматически создаст новые сущности в выбранных ресурсах по заданным правилам. Правила задаются отдельно для каждого ресурса.
...
1. Источник (идентификатор): `{{ .response.id }}`.
1. Источник (название): `{{ .response.name }}`.
...
Если включена опция «Удаление сущностей», портал автоматически удалит сущности в выбранных ресурсах по заданным правилам. Правила задаются отдельно для каждого ресурса.

| Поле     | Описание                                                                      |
| -------- | ----------------------------------------------------------------------------- |
| Ресурс   | Ресурс каталога, в котором находится удаляемая сущность                       |
| Источник | Go-шаблон идентификатора сущности (например, `{{ .response.id }}`)            |

Если сущности с полученным идентификатором нет в ресурсе, удаление пропускается.

Например, при выполнении действия «Удаление проекта в GitLab» в ответе может быть:
...
Чтобы после успешного выполнения действия удалить сущность в ресурсе «Проекты GitLab», укажите в правилах:
...
Если включена опция «Создание связей сущности», портал автоматически создаст новые связи для выбранной сущности по заданным правилам. Набор правил задаётся отдельно для каждого ресурса.
...
Если включена опция «Удаление связей сущности», портал автоматически удалит связи для выбранной сущности по заданным правилам. Набор правил задаётся отдельно для каждого ресурса.

| Поле                                | Описание                                                                                                                                                                                                                                                                                                                                 |
| ----------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Ресурс                              | Любой из двух ресурсов, участвующих в нужной связи ресурсов: по выбранному ресурсу загружается список связей, в которых он указан как родительский или как дочерний. Можно выбрать ресурс сущности, для которой запускается действие, или ресурс другой стороны связи — в списке будет одна и та же связь, если оба ресурса входят в неё  |
| Связь                               | Связь ресурсов в каталоге: в ней заданы родительский и дочерний ресурсы и роли сущностей при удалении связи. По выбранной связи определяется, на какой стороне находится сущность запуска и какое из полей ниже задаёт идентификатор второй сущности из ответа действия                                                                  |
| Идентификатор родительской сущности | Go-шаблон для идентификатора родительской сущности в каталоге                                                                                                                                                                                                                                                                            |
| Идентификатор дочерней сущности     | Go-шаблон для идентификатора дочерней сущности в каталоге                                                                                                                                                                                                                                                                                |

Если подходящей связи нет, удаление пропускается.
...
| Поле     | Описание                                                                                                          |
| -------- | ----------------------------------------------------------------------------------------------------------------- |
| Условие  | Необязательный Go-шаблон; пустое — правило всегда выполняется. Результат рендеринга: `true`, `false`, `1` или `0` |
| Операция | Записать строку, Записать JSON, Добавить строку, Добавить JSON, Слить JSON (поверхностно), Удалить                |
| Цель     | Dot-path в хранилище без ведущей точки, например `deploy.job_id` или `notification.items`                         |
| Источник | Go-шаблон значения                                                                                                |
...
Подробное описание операций, примеры правил, цикл по коллекции и просмотр хранилища — в разделе [«Хранилище процесса»](../processes/store/).
...
Если подтверждение требуется от конкретного пользователя, ему будет отправлено уведомление в веб-интерфейсе.
...
Для встроенных действий портал заранее определяет набор требуемых учётных данных. Их идентификаторы подгружаются при выборе встроенного бэкенда. Для каждого идентификатора необходимо выбрать тип учётных данных, который будет использоваться.
