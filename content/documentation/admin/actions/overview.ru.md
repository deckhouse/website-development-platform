---
title: Обзор
description: Конфигурация действий в Deckhouse Development Platform (DDP) — запросы, форма, обновления сущностей, безопасность и авторизация.
weight: 10
---
...
| `.context`  | Контекст запуска процесса — значения, переданные страницей или карточкой | `{{ .context.repository.id }}`       |
...
| Владелец — инициатор                   | Владельцем создаваемых сущностей становится пользователь, запустивший действие; заданный владелец при этом игнорируется |
...
Если включена опция «Синхронизация источников данных», после успешного выполнения действия платформа синхронизирует выбранные источники. Объект, созданный или удалённый действием во внешней системе, попадёт в каталог сразу, не дожидаясь синхронизации по расписанию.

| Поле            | Описание                                                     |
| --------------- | ------------------------------------------------------------ |
| Источники данных | Источники, которые синхронизируются после выполнения действия |

Синхронизация выполняется в фоне: действие не ждёт её завершения и не считается неуспешным, если синхронизация не удалась.

#### Удаление сущностей

Если включена опция «Удаление сущностей», платформа автоматически удалит сущности в выбранных ресурсах по заданным правилам. Правила задаются отдельно для каждого ресурса.
...
Если включена опция «Удаление связей сущности», платформа автоматически удалит связи для выбранной сущности по заданным правилам. Набор правил задаётся отдельно для каждого ресурса.
