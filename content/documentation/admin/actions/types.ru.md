...
CreateDefectdojoEngagement — создает новую сессию тестирования безопасности (engagement) в системе DefectDojo. Действие использует DefectDojo API v2. Поддерживаемые параметры соответствуют спецификации API.
...
Список полей соответствует официальному API DefectDojo, `/api/v2/engagements`, подробнее в [документации](https://demo.defectdojo.org/api/v2/oa3/swagger-ui/).
...
Список полей соответствует официальному API DefectDojo, `/api/v2/products`, подробнее в [документации](https://demo.defectdojo.org/api/v2/oa3/swagger-ui/).
...
Список полей для переменных соответствует официальному GitLab Group-level Variables API, `/groups/:id/variables`, подробнее в [документации](https://docs.gitlab.com/api/group_level_variables/#create-variable).
...
Список полей для переменных соответствует официальному GitLab Project-level CI/CD variables API, `/projects/:id/variables`, подробнее в [документации](https://docs.gitlab.com/api/project_level_variables/#create-a-variable).
...
* **user** — пользователь, от имени которого будет запускаться выполнение действия.
...
CreateCodeScoringProject — создает новый проект в системе CodeScoring.
...
CreateSonarqubeProject — создает новый проект в SonarQube.
Действие использует SonarQube Web API для создания проекта с указанными параметрами, такими как ключ (project key), название проекта, главная ветка, параметры определения нового кода (new code definition) и видимость проекта.
Аутентификация осуществляется с использованием SonarQube token, который должен быть передан в учетных данных.
...
Для выполнения действия необходимо наличие корректных реквизитов GitLab token.
...
| namespace                 | **обязательно**  | Пространство имён (namespace), в котором находится ресурс                    | -                                                                    |
...
Полный список поддерживаемых параметров – в [официальной документации](https://developer.hashicorp.com/vault/docs/auth/kubernetes#parameters) HashiCorp Vault.
