...
### Реестр шаблонов микросервисов

Набор данных добавляет процессы создания и обновления микросервисов из шаблонов:

- «Создание микросервиса из шаблона»;
- «Обновление микросервиса из шаблона».

Краткое описание в интерфейсе: «Процессы создания и обновления микросервисов из шаблонов».

Настройка:

Перед применением в каталоге должны быть ресурсы систем и микросервисов (например, после применения набора данных «Каноничная архитектура»). Создайте [внешний сервис](../external-services/#gitlab) для GitLab.

Параметры настройки:

| Название           | Описание                                                                                    | Обязательность | Значение по умолчанию |
|--------------------|---------------------------------------------------------------------------------------------|----------------|-----------------------|
| «Внешний сервис»   | Внешний сервис, который будет использоваться набором данных                                 | **да**         | —                     |
| «Провайдер VCS»    | Провайдер системы контроля версий для создания микросервисов. Сейчас доступен только GitLab | **да**         | `gitlab`              |

Применение:

1. Создайте внешний сервис GitLab в разделе «Администрирование» → «Внешние сервисы».
1. Нажмите «Применить» на карточке «Реестр шаблонов микросервисов».
1. Выберите «Внешний сервис» и «Провайдер VCS», затем нажмите «Применить».
1. Настройте источники и синхронизацию в разделе [«Реестр шаблонов»](../template-registry/).

После этого на главной доступны создание микросервиса из шаблона и обновление сервиса из шаблона.

При создании микросервиса параметры сервиса и значения из `values.yaml` шаблона задаются в окнах галереи и передаются в процесс через context; параметры VCS (например «Группа GitLab») — в форме запуска процесса.
