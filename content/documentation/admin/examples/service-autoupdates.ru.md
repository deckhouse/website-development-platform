...
Для реализации автоматического обновления сервисов используются следующие компоненты Deckhouse Development Portal (DDP, портал):
...
- [Источник данных](../datasources/overview/) — механизм DDP, который позволяет синхронизировать информацию из внешних инфраструктурных систем (например, GitLab) в каталог портала. Источники данных периодически опрашивают внешние системы, получают актуальную информацию и обновляют соответствующие сущности в каталоге:
...
- Действие — механизм DDP, который позволяет пользователям портала взаимодействовать с внешними инфраструктурными сервисами через API. Действия могут создавать проекты в GitLab, создавать секреты в системах управления секретами, развёртывать приложения и выполнять другие операции.
...
- Автоматизация — механизм реагирования портала на изменения спецификации сущностей. Автоматизации подписываются на события обновления сущностей ресурсов по определённым правилам (триггерам) и запускают выполнение действий, процессов или синхронизацию источников данных.
...
Настройте правила для создания идентификатора и названия новой сущности в портале:
...
Подробнее о доступных заголовках — в разделе [«GitlabProjects»](../datasources/types/#gitlabprojects) и [«Системная учётная запись»](../datasources/overview/#системная-учётная-запись).
...
Настройка портала для создания сервисов на основе шаблона с последующим автоматическим обновлением при выпуске новой версии шаблона завершена.
