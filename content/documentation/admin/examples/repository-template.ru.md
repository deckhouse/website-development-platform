...
- Создает новый репозиторий с отрендеренным кодом

> Подробнее о шаблонизации в DDP и доступных функциях см. в [документации по шаблонизации](../../user/templating).

Шаблоны репозиториев позволяют стандартизировать структуру проектов, автоматизировать создание новых сервисов и обеспечить единообразие в организации кода между командами.

## Назначение и преимущества

Использование шаблонов репозиториев в DDP предоставляет следующие преимущества:

- **Стандартизация проектов** — единая структура и конфигурация для всех проектов, созданных из шаблона
- **Автоматизация создания сервисов** — быстрое создание новых репозиториев с предварительно настроенной структурой
- **Централизованное управление** — обновления шаблона могут автоматически распространяться на все созданные из него репозитории (см. [инструкцию по автоматическому обновлению сервисов](./service-autoupdates))
- **Снижение ошибок** — минимизация ручных действий при создании новых проектов
- **Ускорение онбординга** — новые разработчики получают готовую структуру проекта с примерами и документацией
...
- **Шаблонизация названий файлов и директорий** — названия файлов и папок могут содержать шаблонные выражения
...
Создайте новый Git-репозиторий в вашей системе управления версиями (например, GitLab). Этот репозиторий будет использоваться как шаблон.
...
Создайте структуру директорий и файлов, которая будет использоваться в проектах, созданных из шаблона. В названиях файлов и директорий можно использовать шаблонные выражения. Структура может быть адаптирована под любой язык программирования или тип проекта.
...
### 3. Добавление шаблонных переменных в файлы

В файлах шаблона используйте переменные в формате Go template для параметризации содержимого. Шаблонизация работает с файлами любого типа и языка программирования.
...
### 4. Создание файла values.yaml

Создайте файл `values.yaml` в корне репозитория для определения переменных по умолчанию. Файл является опциональным, но рекомендуется для предоставления значений по умолчанию и документации доступных переменных. Переменные могут использоваться в файлах любого типа и языка программирования.

> **Важно:** Если файл `values.yaml` отсутствует, все необходимые переменные должны быть переданы при запуске действия создания репозитория из шаблона. При отсутствии переменных одновременно в файле `values.yaml` и в параметрах действия рендеринг завершится с ошибкой, если в шаблоне используются переменные. Подробнее см. в [документации действия CreateRepositoryFromTemplate](../actions/types/#createrepositoryfromtemplate).
...
### 5. Создание файла .templateignore

Если в шаблоне есть файлы или директории, которые содержат шаблонные выражения, но не должны обрабатываться механизмом шаблонизации (например, Helm чарты, которые сами используют Go template), создайте файл `.templateignore` в корне репозитория.
...
### 6. Тестирование шаблона

Перед использованием шаблона в продакшене рекомендуется протестировать его локально с помощью утилиты `ddp-render-dir`. Утилита позволяет проверить корректность рендеринга шаблона с различными значениями переменных.
...
### 7. Коммит и публикация шаблона

После подготовки шаблона закоммитьте все изменения и запушьте их в репозиторий:
...
2. **Настройку действия «Создать репозиторий из шаблона»** — конфигурация действия для использования вашего шаблона
3. **Настройку процесса создания сервиса** — объединение действий в процесс для автоматизированного создания сервисов из шаблона
...
- **Документирование переменных** — создайте файл `README.md` в шаблоне с описанием всех доступных переменных и их назначения
- **Предоставление значений по умолчанию** — используйте файл `values.yaml` для предоставления разумных значений по умолчанию
- **Минимализм** — включайте в шаблон только необходимые файлы и структуру, избегая избыточности
- **Тестирование** — регулярно тестируйте шаблон с различными значениями переменных перед публикацией новой версии
- **Версионирование** — используйте теги Git для версионирования шаблона и документируйте изменения между версиями
...
- [Sprig Template Functions](https://masterminds.github.io/sprig/) — справочник доступных функций шаблонизации из библиотеки Sprig
