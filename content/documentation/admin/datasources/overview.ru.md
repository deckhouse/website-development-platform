...
Параметр `datasources.logging.enabled` в конфигурационном файле DDP backend позволяет выводить в stdout, либо скрывать логи запуска источника данных. При значении `true` логи запуска будут выводиться в stdout, при значении `false` логи выводиться не будут.
