...
|tags_search      |Опционально                                 |Строка для поиска тегов по имени. Описание параметра — в [GitLab Tags API](https://docs.gitlab.com/api/tags/#list-project-repository-tags)                                                                                                                                                                               |Строка                                                                        |\-                                                                            |

## GitlabRepositoryActivity

Источник данных типа `GitlabRepositoryActivity` возвращает активность репозиториев GitLab: число открытых merge requests, последний коммит и последний пайплайн каждого проекта.

В отличие от [GitlabProjects](#gitlabprojects), этот источник не описывает проект целиком, а собирает показатели, которые меняются часто и нужны для отображения состояния сервиса.
...
Платформа выполняет запросы к GraphQL API GitLab по URL `/api/graphql` и возвращает для каждого проекта следующие поля:

| Поле                    | Описание                                                     |
| ----------------------- | ------------------------------------------------------------ |
| `path`                  | Полный путь проекта                                          |
| `gitlab_repository_id`  | Числовой идентификатор проекта                               |
| `default_branch`        | Ветка по умолчанию                                           |
| `open_mr_count`         | Число открытых merge requests                                |
| `last_commit_sha`       | Сокращённый SHA последнего коммита ветки по умолчанию        |
| `last_commit_message`   | Заголовок последнего коммита                                 |
| `last_commit_author`    | Автор последнего коммита                                     |
| `last_pipeline_id`      | Идентификатор последнего пайплайна                           |
| `last_pipeline_status`  | Состояние последнего пайплайна в нижнем регистре             |
| `last_pipeline_web_url` | Ссылка на последний пайплайн                                 |

Поля коммита и пайплайна отсутствуют, если у проекта нет коммитов или пайплайнов.
...
| Название  | Обязательность                             | Описание                                                                       | Возможные значения | По умолчанию |
| --------- | ------------------------------------------ | -------------------------------------------------------------------------------- | ------------------ | ------------ |
| all       | Опционально                                | Собирать активность всех репозиториев, к которым есть доступ; `group_ids` игнорируется | true, false        | false        |
| group_ids | Обязательно, если `all` в значении `false` | Идентификаторы групп через запятую                                              | Пример: 1001,1002  | \-           |

### Примечание

Показатели собираются одним запросом GraphQL на страницу проектов, поэтому число обращений к GitLab не растёт пропорционально числу проектов. При значении `all` в `true` источник обходит все доступные проекты инстанса — учитывайте это при выборе расписания синхронизации.
...
Если неизвестно, какие требуются группы и версии, можно попробовать подставить актуальные значения. Есть несколько вариантов, как их посмотреть, например с помощью утилиты `d8 k`: команда `d8 k explain` показывает `version` и `apiGroup` для ресурса.
...
|---------------------|----------------|-------------------------------------------------------------------------------------------|---------------------------------------------------|---------------------------------------|--------------|
| `paginationType`    | Опционально    | Тип пагинации для обработки больших объёмов данных                                        | `none`, `offset`, `cursor`, `page`, `link_header` | `none`                                | `none`       |
