...
<a id="apigitlabauth"></a>
**Аутентификация**

Для работы виджета необходим GitLab token. Этот токен передается через конфигурацию Headers ([подробнее](#apigitlabheaders)).
...
<a id="apigitlabheaders"></a>
**Заголовки**

| Название      | Обязательность  | Описание                                                              | Значение по умолчанию |
|---------------|-----------------|-----------------------------------------------------------------------|-----------------------|
| Private-Token | **обязательно** | Токен для авторизации в GitLab. [Подробнее](#apigitlabauth) | -                     |
...
<a id="codescoringdependenciesauth"></a>

### Аутентификация

Для работы виджета необходимо API токен. Этот токен передаётся через конфигурацию Headers([подробнее](#codescoringdependenciesheaders)).

Подробнее о том, как получить API токен можно узнать в [официальной документации](https://docs.codescoring.ru/on-premise/how-to/api/).
...
<a id="codescoringdependenciesheaders"></a>
**Заголовки**

| Название      | Обязательность  | Описание                                                                                   |Пример        |
|---------------|-----------------|--------------------------------------------------------------------------------------------|--------------|
| Authorization | **обязательно** | API токен для авторизации в CodeScoring. [Подробнее](#codescoringdependenciesauth)         | <токен>      |
...
<a id="codescoringvulnerabilitiesauth"></a>

### Аутентификация

Для работы виджета необходимо API токен. Этот токен передаётся через конфигурацию Headers([подробнее](#codescoringvulnerabilitiesheaders)).
...
<a id="codescoringvulnerabilitiesheaders"></a>
**Заголовки**

| Название      | Обязательность  | Описание                                                                                   |Пример        |
|---------------|-----------------|--------------------------------------------------------------------------------------------|--------------|
| Authorization | **обязательно** | API токен для авторизации в CodeScoring. [Подробнее](#codescoringvulnerabilitiesauth)      | <токен>      |
...
<a id="defectdojoproductfindingsauth"></a>

### Аутентификация

Для работы виджета необходим API v2 Key токен. Этот токен передается через конфигурацию Headers ([подробнее](#defectdojoproductfindingsheaders)).
...
<a id="defectdojoproductfindingsheaders"></a>
**Заголовки**

| Название      | Обязательность  | Описание                                                                                   |Пример        |
|---------------|-----------------|--------------------------------------------------------------------------------------------|--------------|
| Authorization | **обязательно** | API v2 Key токен для авторизации в DefectDojo. [Подробнее](#defectdojoproductfindingsauth) |Token <токен> |
...
<a id="defectdojoproductfindingssummaryauth"></a>

### Аутентификация

Для работы виджета необходим API v2 Key токен. Этот токен передается через конфигурацию Headers ([подробнее](#defectdojoproductfindingssummaryheaders)).
...
<a id="defectdojoproductfindingssummaryheaders"></a>
**Заголовки**

| Название      | Обязательность  | Описание                                                                                   |Пример        |
|---------------|-----------------|--------------------------------------------------------------------------------------------|--------------|
| Authorization | **обязательно** | API v2 Key токен для авторизации в DefectDojo. [Подробнее](#defectdojoproductfindingssummaryauth) |Token <токен> |
...
<a id="dockerimagesauth"></a>

### Аутентификация

Аутентификация в docker registry осуществляется с помощью Basic Authentication. Для настройки аутентификации необходимо:

1. Сформировать строку username:password
2. Закодировать строку в Base64. Пример команды: `echo "username:password" | base64`
3. В конфигурации виджета добавить заголовок `Authorization`. [Подробнее](#dockerimagesheaders)
...
<a id="dockerimagesheaders"></a>
**Заголовки**

|Название      |Обязательность |Описание                                                                 |Пример         |
|--------------|---------------|-------------------------------------------------------------------------|---------------|
|Authorization |опционально    | Токен для авторизации в docker regisrty. [Подробнее](#dockerimagesauth) | Basic <токен> |

## Gitlab. Запросы слияния

Виджет позволяет отображать данные о Merge Requests в платформе GitLab и выполнять действия с ними.

<a id="gitlabmergerequestauth"></a>

### Аутентификация

Для работы виджета необходим GitLab token. Этот токен передается через конфигурацию Headers ([подробнее](#gitlabmergerequestsheaders)).
...
<a id="gitlabmergerequestsheaders"></a>
**Заголовки**

| Название      | Обязательность  | Описание                                                              | Значение по умолчанию |
|---------------|-----------------|-----------------------------------------------------------------------|-----------------------|
| Private-Token | **обязательно** | Токен для авторизации в GitLab. [Подробнее](#gitlabmergerequestsauth) | -                     |
...
<a id="gitlabpipelinesauth"></a>

### Аутентификация

Для работы виджета необходим GitLab token. Этот токен передается через конфигурацию Headers ([подробнее](#gitlabpipelinesheaders)).
...
<a id="gitlabpipelinesheaders"></a>
**Заголовки**

| Название      | Обязательность  | Описание                                                          | Значение по умолчанию |
|---------------|-----------------|-------------------------------------------------------------------|-----------------------|
| Private-Token | **обязательно** | Токен для авторизации в GitLab. [Подробнее](#gitlabpipelinesauth) | -                     |
...
<a id="gitlabpipelineeditorauth"></a>

### Аутентификация

Для работы виджета необходим GitLab token. Этот токен передается через конфигурацию Headers ([подробнее](#gitlabpipelineeditorheaders)).
...
<a id="gitlabpipelineeditorheaders"></a>
**Заголовки**

| Название      | Обязательность  | Описание                                                              | Значение по умолчанию |
|---------------|-----------------|-----------------------------------------------------------------------|-----------------------|
| Private-Token | **обязательно** | Токен для авторизации в GitLab. [Подробнее](#gitlabpipelineeditorauth) | -                     |
...
<a id="gitlabpipelinestatsauth"></a>

### Аутентификация

Для работы виджета необходим GitLab token. Этот токен передается через конфигурацию Headers ([подробнее](#gitlabpipelinestatsheaders)).
...
<a id="gitlabpipelinestatsheaders"></a>
**Заголовки**

| Название      | Обязательность  | Описание                                                              | Значение по умолчанию |
|---------------|-----------------|-----------------------------------------------------------------------|-----------------------|
| Private-Token | **обязательно** | Токен для авторизации в GitLab. [Подробнее](#gitlabpipelinestatsauth) | -                     |
...
<a id="gitlabtagsauth"></a>

### Аутентификация

Для работы виджета необходим GitLab token. Этот токен передается через конфигурацию Headers ([подробнее](#gitlabtagsheaders)).
...
<a id="gitlabtagsheaders"></a>
**Заголовки**

| Название      | Обязательность  | Описание                                                       | Значение по умолчанию |
|---------------|-----------------|----------------------------------------------------------------|-----------------------|
| Private-Token | **обязательно** | Токен для авторизации в GitLab. [Подробнее](#gitlabtagsauth)   | -                     |
...
<a id="gitlabcontributorsauth"></a>

### Аутентификация

Для работы виджета необходим GitLab token. Этот токен передается через конфигурацию Headers ([подробнее](#gitlabcontributorsheaders)).
...
<a id="gitlabcontributorsheaders"></a>
**Заголовки**

| Название      | Обязательность  | Описание                                                             | Значение по умолчанию |
|---------------|-----------------|----------------------------------------------------------------------|-----------------------|
| Private-Token | **обязательно** | Токен для авторизации в GitLab. [Подробнее](#gitlabcontributorsauth) | -                     |
...
Для работы виджета необходим токен для аутентификации в Kubernetes API. Этот токен передается через конфигурацию Headers ([подробнее](#helmreleasesheaders)).

Подробнее о том, как получить токен можно узнать в [официальной документации](https://kubernetes.io/docs/reference/access-authn-authz/authentication/).
...
Для работы виджета необходим токен для авторизации с Kubernetes API Server. Этот токен передается через конфигурацию Headers ([подробнее](#kubernetesdeploymentsheaders)).
...
Для работы виджета необходим токен для авторизации с Kubernetes API Server. Этот токен передается через конфигурацию Headers ([подробнее](#kubernetesingressesheaders)).
...
<a id="kubernetespodsauth"></a>

### Аутентификация

Для работы виджета необходим токен для авторизации с Kubernetes API Server. Этот токен передается через конфигурацию Headers ([подробнее](#kubernetespodsheaders)).
...
<a id="kubernetespodsheaders"></a>
**Заголовки**

| Название      | Обязательность  | Описание                                                               | Пример           |
|---------------|-----------------|------------------------------------------------------------------------|------------------|
| Authorization | **обязательно** | Токен для авторизации в Kubernetes API. [Подробнее](#kubernetespodsauth) | Bearer <токен> |
...
1. Сформировать строку username:password
2. Закодировать строку в Base64. Пример команды: `echo "username:password" | base64`
3. В конфигурации виджета добавить заголовок `Authorization`. [Подробнее](#nexusartifactsheaders)
...
<a id="opensearchindexauth"></a>

### Аутентификация

Виджет поддерживает механизм Basic Authentication для доступа к OpenSearch. Для настройки аутентификации необходимо:

1. Сформировать строку username:password
2. Закодировать строку в Base64. Пример команды: `echo "username:password" | base64`
3. В конфигурации виджета добавить заголовок `Authorization`. [Подробнее](#opensearchindexheaders)
...
<a id="opensearchindexheaders"></a>
**Заголовки**

| Название      | Обязательность     | Описание                                                              | Пример        |
|---------------|--------------------|-----------------------------------------------------------------------|---------------|
| Authorization    | **обязательно** | Токен для авторизации в OpenSearch. [Подробнее](#opensearchindexauth) | Basic <токен> |
...
<a id="prometheusmetricsrangeleheaders"></a>
**Заголовки**

| Название      | Обязательность  | Описание                                                           |Значение по умолчанию |
|---------------|-----------------|--------------------------------------------------------------------|----------------------|
| Заголовки     | опционально     | Заголовки для доступа к Prometheus                                 | -                    |
...
<a id="prometheusmetricssingleheaders"></a>
**Заголовки**
...
<a id="sonarqubeauth"></a>

### Аутентификация

Для работы виджета необходим Sonarqube токен с типом User Token. Этот токен передается через конфигурацию Headers ([подробнее](#sonarqubeheaders)).
...
<a id="sonarqubeheaders"></a>
**Заголовки**

| Название      | Обязательность | Описание                                                       | Пример         |
|---------------|----------------|-----------------------------------------------------------------|----------------|
| Authorization | опционально    | Токен для авторизации в Sonarqube. [Подробнее](#sonarqubeauth) | Basic <токен>  |
...
<a id="kubernetesresourcequotasauth"></a>

### Аутентификация

Для работы виджета необходим токен для авторизации с Kubernetes API Server. Этот токен передается через конфигурацию Headers ([подробнее](#kubernetesresourcequotasheaders)).
...
<a id="kubernetesresourcequotasheaders"></a>
**Заголовки**

| Название      | Обязательность  | Описание                                                                           | Пример         |
|---------------|-----------------|------------------------------------------------------------------------------------|----------------|
| Authorization | **обязательно** | Токен для авторизации в Kubernetes API. [Подробнее](#kubernetesresourcequotasauth) | Bearer <токен> |
...
Для работы виджета необходим [Kaiten API Token](https://developers.kaiten.ru/). Токен передается через заголовок **Authorization**.
