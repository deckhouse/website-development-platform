...
Конфигурация авторизации описана в разделе [внешний сервис Docker Registry](../external-services/#docker-registry).
...
| Название        | Обязательность  | Описание                                                                                           | Значение по умолчанию |
|-----------------|-----------------|----------------------------------------------------------------------------------------------------|-----------------------|
| Название релиза | **обязательное**| Имя релиза, отображаемое в списке.                                                                  | -                     |
...
Конфигурация авторизации описана в разделе [внешний сервис OpenSearch](../external-services/#opensearch).
...
Виджет позволяет отображать сущности выбранного ресурса на временной шкале.

### Отображаемые данные

* **График временной шкалы** - горизонтальная диаграмма, где каждая сущность представлена полосой, показывающей период времени
* **Информация о сущностях** - при наведении на полосу отображается название сущности, дата начала и дата окончания
...
Виджет позволяет отображать сущности выбранного ресурса в календаре.
...
* **Информация о сущностях** - для каждой сущности отображается название и описание (если указано)
...
Виджет позволяет отображать информацию о статусе сущности и результатах проверок статуса.
...
- **Прогресс-бар** — визуальное отображение общего статуса сущности с процентным соотношением успешных проверок
- **Счетчик успешных проверок** — количество пройденных проверок из общего числа
...
- **Статус** — результат проверки:
  ...
  - **Не пройдено** — проверка не пройдена (без ошибки выполнения)
  - **Ошибка** — при выполнении проверки произошла ошибка
- **Время последней проверки** — дата и время последнего выполнения проверки
- **Сообщение об ошибке** — текст ошибки (если проверка завершилась с ошибкой)

#### Статистика

В нижней части виджета отображается статистика:

- **Пройдено** — количество успешно пройденных проверок
- **Не пройдено** — количество непройденных проверок (без ошибок)
- **Ошибка** — количество проверок, завершившихся с ошибкой

#### Заблокированные действия

Виджет автоматически определяет и отображает действия, которые заблокированы текущим статусом сущности:

- **Условия отображения:**
  - Действие должно быть доступно для ресурса сущности
  - У действия должны быть настроены разрешенные статусы
  - Текущий статус сущности не входит в список разрешенных статусов действия
...
Виджет не требует дополнительной конфигурации. Для работы виджета необходимо:

- Настроить проверки статуса для ресурса сущности ([подробнее](../healthchecks/overview/))

### Особенности

- Если для сущности не настроено ни одной проверки статуса, виджет отображает соответствующее сообщение
...
Конфигурация авторизации описана в разделе [внешний сервис Kaiten](../external-services/#kaiten).
