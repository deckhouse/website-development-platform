...
| Протокол аутентификации | Да         | Протокол для подключения к Kafka. [Список протоколов в документации Kafka](https://kafka.apache.org/documentation/#adminclientconfigs_security.protocol)                                                                                | -                     |
| Механизм SASL           | Нет            | Механизм аутентификации, который будет использовать SASL. Обязателен при использовании протокола SASL_PLAINTEXT или SASL_SSL. [Список механизмов SASL в документации Kafka](https://kafka.apache.org/documentation/#security_sasl_mechanism) | -                     |
...
Виджет показывает данные снимка ветки в Svacer: обзор метрик, распределение находок и таблицу маркеров.
...
| Название ветки                    | **да**                                   | Название ветки в Svacer; используется, если в параметрах запроса не выбрана другая ветка                  | —                     |
| Последний снимок                  | нет                                      | Загружать данные из последнего снимка на ветке                                                            | включено              |
| Название снимка                   | **да**, если «Последний снимок» выключен | Название снимка в Svacer                                                                                  | —                     |
| Кэшировать полный список маркеров | нет                                      | Кэширует на DDP полный список маркеров выбранного снимка; ускоряет пагинацию и снижает нагрузку на Svacer | включено              |
| Время жизни кэша (сек.)           | нет                                      | Сколько секунд хранить кэш в памяти DDP (30–86400)                                                        | 180                   |
| Порог маркеров в снимке           | нет                                      | Число маркеров, после которого снимок считается большим                                                   | 10 000                |
| Стратегия для больших снимков     | нет                                      | Поведение при превышении порога: «Без ограничений» или «Гибрид»                                           | «Гибрид»              |

#### Дополнительные возможности виджета

Виджет содержит вкладки «Обзор» и «Находки».

На вкладке «Обзор» отображаются показатели «Прогресс ревью», «Ожидают ревью» и «Срабатываний на ветке», сравнение с предыдущим снимком, а также диаграммы «Распределение по критичности» и «Динамика срабатываний».

На вкладке «Находки» — таблица маркеров с пагинацией и ссылкой «Открыть» в Svacer.

При стратегии «Гибрид» и снимке, превышающем порог маркеров, список находок без фильтров может быть недоступен; доступна кнопка «Загрузить все находки».

#### Параметры запроса

| Название     | Обязательность | Описание                                                                           | Значение по умолчанию |
|--------------|----------------|------------------------------------------------------------------------------------|-----------------------|
| Ветка        | нет            | Ветка Svacer для загрузки данных                                                   | из конфигурации       |
| Снимок       | нет            | Снимок ветки; «Последний снимок» — актуальный снимок в момент обновления виджета   | из конфигурации       |
| Статус ревью | нет            | Фильтр маркеров по статусу ревью                                                   | —                     |
| Критичность  | нет            | Фильтр по критичности (вкладка «Находки»)                                          | —                     |
| Чекер        | нет            | Идентификатор чекера в Svacer (например, gosec.G402); фильтр на вкладке «Находки»  | —                     |
