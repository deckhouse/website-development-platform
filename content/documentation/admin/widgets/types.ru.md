...
Виджет показывает список Application в Argo CD: сводку по состоянию, фильтры, таблицу приложений и состав выбранного Application (управляемые ресурсы, отличия, манифест, логи подов).

#### Авторизация

Конфигурация авторизации описана в разделе [«Внешние сервисы»](../external-services/#argo-cd).

#### Конфигурация

| Название                      | Обязательность | Описание                                                                                                      | Значение по умолчанию |
|-------------------------------|----------------|---------------------------------------------------------------------------------------------------------------|-----------------------|
| URL                           | **да**         | Базовый URL сервера Argo CD                                                                                   | -                     |
| Проекты Argo CD               | нет            | По одному названию проекта на строку; пусто — все проекты, доступные токену                                  | -                     |
| Название приложения содержит  | нет            | Подстрочный фильтр без учёта регистра по `metadata.name` (если не включено «Регулярное выражение»). Примеры: `prod` — «backend-prod», «prod-api»; `guest-` — «guest-booking»; пусто — без фильтра по названию | -                     |
| Регулярное выражение          | нет            | Если **да**, поле «Название приложения содержит» — шаблон Go regexp (RE2) по `metadata.name`. Примеры: `^prod-`, `staging$` | нет                   |

#### Использование виджета

На панели виджета отображаются карточки «Приложения», «Деградация» и «Не синхронизировано» — нажатие переключает соответствующий фильтр. Доступны поиск, фильтры по проекту, статусу Sync и Health, быстрый фильтр («Все», «Ошибки», «Не синхронизировано»).

В таблице для каждого Application можно открыть «Состав», запустить синхронизацию, включить или отключить автоматическую синхронизацию, перейти в Argo CD или к коммиту. Для нескольких выбранных строк доступны массовые действия: синхронизация, включение и отключение автоматической синхронизации.

В панели состава доступны просмотр манифеста и server-side отличий ресурса, общий diff Application, логи пода и синхронизация Application.

{{< alert level="info" >}}
Действия с Application требуют включённой функции действий в настройках виджета и соответствующих прав токена в Argo CD.
{{< /alert >}}

### Argo CD. Приложение

Виджет показывает состав одного Application: дерево управляемых ресурсов и те же операции, что в виджете «Argo CD. Приложения», без таблицы списка.
...
|--------------------------------|----------------|---------------------------------------------------------------------------------------------------------------|-----------------------|
| URL                            | **да**         | Базовый URL сервера Argo CD                                                                                   | -                     |
| Название Application           | **да**         | Точное совпадение с `metadata.name` ресурса Application в Kubernetes                                          | -                     |
| Namespace Application          | **да**         | Точное совпадение с `metadata.namespace` ресурса Application в Kubernetes                                     | -                     |
| Проект Argo CD (необязательно) | нет            | При указании сравнивается без учёта регистра с полем `project` в spec Application                             | -                     |

Если Application не найдено или недоступно токену, отображается предупреждение.

### Argo CD. История синхронизаций

Виджет показывает хронологию синхронизаций и последних операций по Application с фильтрами и ссылками на Argo CD.
...
| Название приложения содержит  | нет            | Подстрочный фильтр без учёта регистра по `metadata.name` (если не включено «Регулярное выражение»). Примеры: `prod` — «backend-prod», «prod-api»; `guest-` — «guest-booking»; пусто — без фильтра по названию | -                     |
| Регулярное выражение          | нет            | Если **да**, поле «Название приложения содержит» — шаблон Go regexp (RE2) по `metadata.name`. Примеры: `^prod-`, `staging$`                                                                                   | нет                   |
| Максимум строк истории        | нет            | Максимальное число строк в таблице (сначала новые) по истории каждого приложения и последней операции                                                                                                         | 80                    |

#### Использование виджета

Записи сгруппированы по Application. Доступны поиск, быстрый фильтр («Все», «Только проблемы»), фильтр по приложению и кнопка «Очистить».

### Argo CD. Аналитика времени деплоя

Виджет показывает метрики длительности синхронизаций за выбранный период (24 ч, 7 сут, 30 сут): среднюю длительность, тренд к предыдущему периоду, долю успеха, гистограмму распределения и таблицы по приложениям.
...
| Название приложения содержит                             | нет            | Подстрочный фильтр без учёта регистра по `metadata.name` (если не включено «Регулярное выражение»). Примеры: `prod` — «backend-prod», «prod-api»; `guest-` — «guest-booking»; пусто — без фильтра по названию | -                     |
| Регулярное выражение                                     | нет            | Если **да**, поле «Название приложения содержит» — шаблон Go regexp (RE2) по `metadata.name`. Примеры: `^prod-`, `staging$`                                                                                   | нет                   |
| Верхние границы корзин гистограммы (с)                   | нет            | По одному значению в строке: верхняя граница интервала в секундах (не включительно), по возрастанию; пусто — 60, 180, 300                                                                                     | 60, 180, 300          |
| Размер списка по максимальной длительности синка (строк) | нет            | Сколько приложений в рейтинге по максимальной длительности синка в окне; 0 — 5; не более 10                                                                                                                   | 5                     |
| Минимальное отклонение для аномалий (с)                  | нет            | Приложение попадает в таблицу аномалий, если последний деплой в окне медленнее медианы остальных деплоев минимум на столько секунд                                                                            | 120                   |
| Время кэширования ответа (с)                             | нет            | Сколько секунд ответ хранится на сервере без повторного запроса к Argo CD; 0 — без кэша                                                                                                                       | 300                   |

#### Использование виджета

Кнопка «Таблицы по приложениям» открывает вкладки «Макс. длительность» и «Аномалии». В таблице аномалий кнопка «История синхронизаций» открывает диалог с записями по выбранному Application.

Если список приложений усечён на стороне сервера, под заголовком виджета отображается соответствующая подсказка.
...
При **создании**, **слиянии** и **закрытии** Pull Request в GitHub автором PR и исполнителем действий считается учётная запись GitHub, которой принадлежит этот токен. Логин и имя в интерфейсе GitHub могут не совпадать с именем в профиле DDP.
...
Виджет позволяет создавать теги в Bitbucket напрямую из платформы DDP.
...
| Шаг разрешения (сек.) | **Да**         | Интервал между отсчетами на горизонтальной оси (в секундах)                                                                          | 60                    |
| Метка                 | **Да**         | Метка в результатах запроса, чье уникальное значение присваивается в качестве названий соответствующих линий на графике визуализации | -                     |
| Порог                 | Нет            | Порог, отображаемый в виде горизонтальной красной полосы на графике                                                                  | -                     |
| Минимальное значение  | Нет            | Начальная точка отсчета для вертикальной линии на графике                                                                            | -                     |
| Максимальное значение | Нет            | Предельная точка отсчета для вертикальной линии на графике                                                                           | -                     |
...
| Порог успеха (%)                 | Нет            | Граница между оранжевым и зелёным цветами. Если значение метрики превышает этот процент от порога, оно получит зеленый цвет   | 90                    |
| InsecureSkipVerify               | Нет            | Отключение проверки подлинности TLS/SSL-сертификата Prometheus                                                                | false                 |
...
{{< alert level="info" >}}
Виджет не передает значения секретов пользователю. На клиентскую сторону передаются только метаданные секретов (версия, время создания и т.д.) и структура ключей без их значений.
{{< /alert >}}
...
- **Время удаления** — дата и время удаления секрета (для удаленных версий).
