...
Виджет позволяет просматривать секреты в HashiCorp Vault или Deckhouse Stronghold, организованные в иерархическую структуру. Виджет поддерживает работу с KV v2 секретами.
...
* Просмотр ключей секрета в формате таблицы "ключ/значение" (значения маскируются).
* Навигация по вложенным секретам и директориям.
* Ленивая загрузка содержимого директорий и ключей секретов.
...
{{< alert level="info" >}}
Виджет использует заголовки HTTP для авторизации, аналогично другим виджетам (например, Kubernetes). Токен авторизации передается через заголовок `X-Vault-Token` или другой заголовок, настроенный во внешнем сервисе.
{{< /alert >}}
...
| Название   | Опциональность  | Описание                                                                                                                                    | Значение по умолчанию |
|------------|-----------------|---------------------------------------------------------------------------------------------------------------------------------------------|-----------------------|
| Путь       | **обязательно** | Путь к секрету или директории в Vault. Для KV v2 необходимо явно указывать путь с `/data/`. Примеры: `services/data/`, `services/data/example` | -                     |
...
* **Директории** — отображаются с завершающим слэшем (например, `nested/`) и всегда помечаются как директории, даже если содержат ключи.
* **Секреты** — отображаются без завершающего слэша и содержат ключи.
