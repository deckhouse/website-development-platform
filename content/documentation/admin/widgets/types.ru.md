...
### Активность разметки Svacer

Виджет показывает, кто и сколько размечает находки в Svacer за выбранный период: список ревьюеров, карточку выбранного ревьюера с распределением вердиктов и активностью по дням, а также журнал операций разметки с выгрузкой в CSV.
...
| Название                          | Обязательность | Описание                                                                                                                                                              | Значение по умолчанию |
|-----------------------------------|----------------|-----------------------------------------------------------------------------------------------------------------------------------------------------------------------|-----------------------|
| Период по умолчанию (дней)        | нет            | С каким периодом виджет открывается; читатель может переключить период в самом виджете. От 1 до 30 дней, сутки считаются по UTC                                       | 30                    |
| План операций разметки за 30 дней | нет            | Сколько операций разметки ожидается от одного ревьюера за 30 дней. В более узком периоде план пересчитывается пропорционально; если поле пустое, план не показывается | -                     |
| Показывать ФИО                    | нет            | Подставляет имена и email вместо логинов; без административного токена Svacer будут показаны логины                                                                   | Включено              |
| Кэшировать журнал разметки        | нет            | Если выключено, журнал загружается из Svacer при каждом запросе виджета                                                                                               | Включено              |
| Время жизни кэша (секунды)        | нет            | Сколько секунд виджет может показывать уже загруженные данные, не обращаясь к Svacer. Допустимый диапазон при указании значения: 30–86400                             | Значение инстанса     |

#### Параметры запроса

При просмотре виджета доступны следующие параметры:

* Период — «7 дней», «14 дней», «30 дней» или «Абсолютный диапазон времени» с границами «от» и «до».
* «Все проекты» — проекты Svacer, найденные в журнале за период; если ни один проект не выбран, учитываются все.

#### Просмотр данных

* Список ревьюеров — «Поиск по ФИО или логину», сортировка «по объёму» или «по алфавиту». Если задан план, рядом с числом операций показывается процент его выполнения.
* Карточка ревьюера — «Операций», «План», «Активность», «По дням», распределение вердиктов и «Где размечал». Клик по столбику в «По дням» фильтрует журнал по этому дню.
* Метрика — «Операции» или «Маркеры». «Операции» — транзакции разметки за период, то есть выполненная работа. «Маркеры» — находки, на которых сейчас стоит вердикт ревьюера; они считаются по одной ветке, поэтому ветку нужно выбрать в блоке «Ветка».
* Таблица — в метрике «Операции» колонки «Время», «Проект», «Вид», «Комментарий» и «Что поставлено», в метрике «Маркеры» — «Файл», «Детектор», «Размечено», «Сообщение» и «Вердикт». Раскрытие строки операции показывает маркеры, которых она коснулась.
* «Скачать .csv» — выгрузка строк таблицы с учётом выбранных фильтров.

{{< alert level="info" >}}
Если журнал за период слишком велик, виджет показывает только последние операции и предупреждает об этом; числа в карточке ревьюера при этом занижены. Чтобы увидеть полные данные, сузьте период.
{{< /alert >}}
