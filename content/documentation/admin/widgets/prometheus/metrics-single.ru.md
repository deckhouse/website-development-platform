...
description: Одиночное значение метрики на основе PromQL-запроса к Prometheus.
weight: 20
---

Виджет показывает одно число по PromQL-запросу к Prometheus.
Запрос должен возвращать тип `Scalar` или тип `Vector` с одним значением.
