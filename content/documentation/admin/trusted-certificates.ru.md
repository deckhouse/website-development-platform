...
Доверенные сертификаты позволяют добавить в хранилище доверия DDP корневые и промежуточные сертификаты (CA) или сертификаты серверов. Платформа использует их при проверке TLS/SSL при подключении к внешним сервисам по HTTPS — например, при обращении к API внешних сервисов, источников данных, виджетов и действий.
...
Добавленные доверенные сертификаты автоматически подмешиваются в пул корневых сертификатов, используемый DDP Backend при исходящих HTTPS-запросах. Этот пул применяется при:
