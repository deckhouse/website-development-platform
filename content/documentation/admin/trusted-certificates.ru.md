...
Механизм доверенных сертификатов позволяет загрузить в Deckhouse Development Portal (DDP, портал) корневые и промежуточные сертификаты (CA) или сертификаты серверов. Портал использует их при проверке TLS/SSL при подключении к внешним сервисам по HTTPS — например, при обращении к API внешних сервисов в источниках данных, виджетах и т. д.
...
{{< alert level="info" >}}
Для Dex, PostgreSQL и Redis, используемых порталом в качестве служебных сервисов, сертификаты необходимо добавлять через конфигурацию модуля: соединение с этими компонентами устанавливается до включения механизма доверенных сертификатов.
{{< /alert >}}
...
Таким образом, если внешний сервис использует сертификат, выпущенный вашим или корпоративным CA, достаточно добавить соответствующий корневой или промежуточный сертификат в раздел «Доверенные сертификаты» — после этого портал будет доверять таким соединениям без отключения проверки SSL.
...
Подробнее о ролевой модели [в документации по RBAC](../security/rbac/).
