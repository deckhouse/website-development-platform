...
Контекст запуска — это набор значений, которые страница или карточка передаёт процессу в момент запуска. Он не вводится пользователем и не хранится в конфигурации процесса: его формирует то место интерфейса, откуда процесс запущен.

Например, карточка микросервиса передаёт идентификатор и адрес его репозитория, а раздел заказа ресурсов — систему, для которой заказывается ресурс.

В Go-шаблонах действий процесса контекст доступен через корень `.context`:

```yaml
project_id: '{{ .context.repository.id }}'
```

Состав контекста зависит от места запуска. Если процесс запущен вручную из каталога, контекст пуст, и обращение к его полям вернёт пустое значение.
