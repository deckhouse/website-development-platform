...
1. «Тело цикла» — нижний порт; от него строится ветка, которая выполняется на каждой итерации.
1. «Выход из цикла» — правый порт; по этой ветке выполнение продолжается после последней итерации.
...
Элемент «Шаблон» вычисляет Go-шаблон без вызова внешнего действия и записывает результат в хранилище как строку.
...
- «Подсказка формата» — как отображать значение при просмотре запуска (`text`, `markdown`, `html`, `json`); на содержимое store не влияет.
...
- **Запись по правилам** — данные попадают в store после успешного выполнения действия (раздел [«Обновление хранилища процесса»](../../actions/overview/#обновление-хранилища-процесса)) или при проходе элемента «Шаблон». Доступны операции записи строки и JSON, добавления, слияния и удаления, а также условие на каждое правило.
