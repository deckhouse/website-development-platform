...
Для операций **Записать JSON**, **Добавить JSON** и **Слить JSON** удобно использовать функцию `toJSON` и прямую ссылку на поле ответа, например `{{ .response.items }}` — портал подставит значение без лишней сериализации.
