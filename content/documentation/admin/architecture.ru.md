...
* Подключается к PostgreSQL для хранения и управления постоянными данными

### DDP Worker

**DDP Worker** — это компонент для выполнения фоновых задач и асинхронных операций. Worker обрабатывает задачи из очередей Redis Streams и выполняет длительные операции, не блокируя работу основного API.
