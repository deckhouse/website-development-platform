...
- Условие задаётся одним или несколькими правилами: левое значение (например, результат предыдущей задачи `{{ .prevTask_result }}`), операция (равно, содержит, больше и т.п.) и правое значение. При нескольких правилах выбирается комбинирование **AND** (все должны выполниться) или **OR** (достаточно одного). Выполняется одна из двух исходящих веток — «при истине» или «при ложности».
