...
Результаты выполненных задач можно передавать в последующие шаги процесса: в настройках действия задаются правила «Response to store» (Source — шаблон по response, Target — ключ в store), а в конфигурации следующих действий используются плейсхолдеры `{{ .store.<ключ> }}`. Подробнее см. [Хранилище для действий в BPMN](../user/processes-store/).
