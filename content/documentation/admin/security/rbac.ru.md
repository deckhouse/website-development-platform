...
{{< alert level="info" >}}
Разрешение `update:team-variables` позволяет редактировать переменные только тех команд, участником которых является пользователь. Даже если пользователь супер-администратор, он не сможет изменить переменные команд, в которых не состоит.
{{< /alert >}}
