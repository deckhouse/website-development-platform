...
Ролевая модель Deckhouse Development Portal (DDP, портал) определяет, какие действия пользователи могут выполнять в портале и к каким объектам они имеют доступ. Модель используется для разграничения прав при работе с API и веб-интерфейсом и применяется на уровне портала в целом и отдельных объектов.
...
{{< alert level="info" >}}
Если у пользователя нет соответствующих разрешений, он не сможет выполнять никакие операции на портале. Система проверяет разрешения для каждой операции.
{{< /alert >}}
...
Глобальные разрешения действуют на уровне всего портала и предоставляют доступ ко всем объектам определенного типа:
...
MCP:
- `read:mcp-servers` — просмотр подключённых MCP-серверов.
- `edit:mcp-servers` — подключение, редактирование, синхронизация каталога и удаление MCP-серверов.
- `read:mcp-collections` — просмотр MCP-коллекций.
- `edit:mcp-collections` — создание, редактирование и удаление MCP-коллекций.
- `read:mcp-tools` — просмотр пользовательских MCP-инструментов.
- `edit:mcp-tools` — создание, редактирование и удаление пользовательских MCP-инструментов.
...
Разрешения `view:` контролируют доступ к различным разделам интерфейса портала:
...
- `use:mcp-collections` — вызов инструментов коллекции в AI-ассистенте и через MCP-сервер портала.
...
{{< alert level="info" >}}
Проверка владения включается только если в конфигурации портал включена опция `ownerIsAdmin`. Подробнее о владении объектами — в разделе [«Владение объектами»](#владение-объектами).
{{< /alert >}}
...
{{< alert level="info" >}}
Опция `ownerIsAdmin` должна быть настроена администратором портала в конфигурационном файле. По умолчанию опция выключена.
{{< /alert >}}

### Автоматическое назначение владельца при создании

При создании нового объекта (действия, источника данных, виджета, ресурса и других объектов) портал автоматически устанавливает текущего пользователя в качестве владельца объекта. При необходимости можно переназначить владельца, либо создать объект без владельца.
...
Портал позволяет назначить одну глобальную роль по умолчанию, разрешения к которой применяются ко всем авторизованным пользователям. Настройка выполняется в разделе «Администрирование → Управление доступом» с помощью переключателя в таблице «Роли».
...
Портал предоставляет готовые пресеты ролей для быстрой настройки типовых сценариев доступа:
...
- Назначение: инженеры, которые настраивают портал (процессы, источники данных, дашборды и т.д.).
