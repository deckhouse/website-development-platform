...
- Разрешения:
  - **Чтение**:
    - `read:actions`, `read:automations`, `read:dashboards`.
    - `read:external-services`, `read:processes`, `read:webhooks`.
    - `read:widgets`, `read:workflows`.
  - **Выполнение**:
    - `run:actions` — запуск действий.
    - `run:widget-actions` — запуск действий виджетов.
    - `control:processes` — управление процессами.
...
- Тип: `global`.
- Разрешения: Чтение информации и выполнение действий (используйте пресет "Developer").
- Назначение: Разработчики, которым нужен доступ к просмотру информации и выполнению действий.
