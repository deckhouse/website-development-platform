...
Лимиты выгрузки (максимальное число строк в одном файле, размер батча чтения из БД при формировании ответа) и идентификатор узла для метаданных экспорта задаются в конфигурации DDP Backend и при необходимости дополняются переменными окружения (например, имя узла для поля источника выгрузки).
