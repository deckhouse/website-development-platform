...
Deckhouse Development Portal (DDP, портал) предоставляет набор встроенных механизмов безопасности, обеспечивающих контроль доступа, защиту учётных данных и аудит действий пользователей при работе с порталом.

## Аутентификация

Аутентификация пользователей в DDP выполняется с использованием внешнего сервиса Dex, развернутого в составе Deckhouse Platform.
...
В DDP реализована ролевая модель доступа, обеспечивающая разграничение прав пользователей и контроль операций, выполняемых в портале.
...
Аудит-логи предназначены для регистрации действий пользователей и контроля операций, выполняемых через API портала. Формирование аудит-логов выполняется на стороне DDP Backend.
...
Аудит-логи сохраняются в базе данных PostgreSQL и доступны для просмотра через веб-интерфейс портала. Также возможна выгрузка аудит-логов в CSV из интерфейса портала.
Подробнее — [в документации по аудит-логам](../audit-logs/).

## HTTP-заголовки безопасности

Для защиты веб-интерфейса и API DDP используются стандартные HTTP-заголовки безопасности.
...
В портале применяются следующие HTTP-заголовки безопасности:
