...
DDP позволяет добавлять в хранилище доверия корневые и промежуточные сертификаты (CA) или сертификаты серверов. Они используются при проверке TLS при исходящих HTTPS-запросах к внешним сервисам (действия, виджеты, источники данных и др.). Управление доверенными сертификатами выполняется через веб-интерфейс в разделе «Администрирование» → «Доверенные сертификаты».
