...
- Vault — значения хранятся в HashiCorp Vault или в Deckhouse Stronghold. Доступно только если в портале включена и настроена интеграция с Vault.
...
Если для типа учётных данных выбрано хранилище Vault, то реквизиты пользователей сохраняются порталом в HashiCorp Vault или в Deckhouse Stronghold.
...
1. Включите использование Vault (Включено).
1. Заполните параметры:
   ...
   - «Путь» — путь к секретам в KV v2. Для KV v2 путь должен содержать сегмент `/data/` (например, `secrets/data/ddp` или `secrets/data/ddp/credentials`). Mount (например, `secrets`) должен быть заранее создан в Vault; подкаталоги после `data/` создаются порталом автоматически при сохранении учётных данных.
1. Нажмите «Проверить подключение», чтобы убедиться, что DDP может аутентифицироваться и обращаться к Vault.
...
## Интеграция в компоненты портала
...
1. Извлекает учётные данные этого пользователя из базы данных.
1. Расшифровывает значения учётных данных.
...
1. Извлекает и расшифровывает учётные данные системного пользователя.
...
1. Извлекаются и расшифровываются учётные данные.
1. Значения подставляются в конфигурацию виджета.
...
1. Приоритет имеют учётные данные, указанные непосредственно в конфигурации компонента.
