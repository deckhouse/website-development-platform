...
Проверка правил по ресурсу производится в те минуты, когда по расписанию должно выполниться хотя бы одно включённое правило. Расписание у правила можно не заполнять: тогда по умолчанию правило участвует в проверке каждую минуту. Логи последних нескольких проверок доступны в меню ресурса в разделе «Проверки статуса».

<a id="health-rule-schedule-cron"></a>

### Расписание проверки (cron)

На вкладке «Основная информация» в поле «Расписание проверки (cron)» задаётся стандартное cron-выражение из пяти полей в UTC. Если поле оставить пустым, проверка этого правила выполняется каждую минуту. Пример: «0 * * * *» — в начале каждого часа. В минуты, когда правило по расписанию не выполняется, для него сохраняется результат последнего расчёта.
