...
В состав Deckhouse Development Portal (DDP, портал) входят следующие компоненты.

### DDP Frontend

DDP Frontend — это веб-интерфейс портала, построенный на Vue.js 3 с TypeScript. Компонент предоставляет пользователям графический интерфейс для работы с порталом через веб-браузер.
...
DDP Backend — это серверная часть портала, реализованная на языке Go. Компонент предоставляет REST API для управления всеми механизмами портала.

Основные функции:

* Предоставление REST API для программного взаимодействия с порталом.
* Обработка бизнес-логики портала.
...
{{< alert level="warning" >}}
Dex не входит в состав модуля DDP и является частью Deckhouse Platform (DP).
{{< /alert >}}
...
Redis — это хранилище данных в памяти, используемое порталом для работы с очередями задач.
...
PostgreSQL — это основная реляционная база данных портала, используемая для хранения всей постоянной информации.
...
### Взаимодействие между компонентами портала
