...
- удаление перенесённых записей из очереди в PostgreSQL и перестроение её индексов;
...
Параметры этих операций задаются в секции `clickhouse.replication` настроек модуля: размер и периодичность переноса, периодичность и отсрочка очистки очереди, перестроение индексов и интервал удаления истории несуществующих объектов.
