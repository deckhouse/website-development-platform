...
Deckhouse Development Portal (DDP, портал) — это IDP (Internal Developer Platform), внутренний портал разработки, который объединяет в себе инструменты, инфраструктурные сервисы и процессы разработки.

Портал предоставляет единый слой взаимодействия с инфраструктурными и инженерными сервисами и используется для автоматизации операций, связанных с жизненным циклом продуктов.
...
Команды разработки используют DDP для работы с сущностями и ресурсами портала.
...
Они настраивают интеграции с внешними сервисами, конфигурируют ресурсы, создают шаблоны и автоматизации, управляют окружениями и обеспечивают работу портала.
...
В рамках портала они настраивают политики безопасности и права доступа, просматривают аудит-логи и управляют механизмами защиты.

### Администраторы

Администраторы — пользователи, обладающие полными правами на настройку и сопровождение портала.
Они управляют учётными записями пользователей, настраивают роли и права доступа через систему RBAC, конфигурируют интеграции с внешними сервисами, управляют учётными данными, ресурсами, автоматизациями и параметрами портала, а также просматривают аудит-логи.
...
Все группы пользователей получают доступ к порталу через веб-интерфейс DDP Frontend, предназначенный для работы с порталом в веб-браузере.
...
Администраторы и разработчики могут напрямую обращаться к DDP Backend через REST API для программного взаимодействия с порталом.
...
REST API предоставляет полный набор функций для управления всеми объектами портала: сущностями, ресурсами, действиями, источниками данных, виджетами, процессами и другими компонентами.
...
Типичный сценарий: Пользователи получают доступ к порталу через внутреннюю сеть компании.
...
Типичный сценарий: Пользователи получают доступ к порталу извне корпоративной сети.
...
Портал может быть настроен для поддержки обоих сценариев одновременно:
...
Конфигурация доступа настраивается администратором при развёртывании портала через параметры Ingress-контроллера и сетевые политики Kubernetes.
