...
1. Заполните форму:
   ...
   - **Заголовки**: добавьте заголовок авторизации:
     ...
     > **Безопасность**: Все чувствительные заголовки (Authorization, X-API-Key, API-Key, X-Auth-Token, Auth-Token) автоматически шифруются при сохранении провайдера. Вы можете указывать ключи напрямую в заголовках — они будут безопасно зашифрованы.

     > **Рекомендация**: Для еще большей безопасности и удобства используйте [систему управления учетными данными AI-провайдеров](../ai-credentials/), которая позволяет хранить ключи отдельно и использовать плейсхолдеры `{{credential:key}}` в заголовках.
...
> **Безопасность**: Чувствительные заголовки (Authorization, X-API-Key, API-Key, X-Auth-Token, Auth-Token) автоматически шифруются при сохранении провайдера.
