...
В настройках провайдера можно задать **кастомные промпты** — дополнительные инструкции для агента (роль, ограничения, стиль ответов и т.п.). Они добавляются в начало системного промпта перед основными правилами и описанием инструментов.

**Как настроить:**

1. В форме создания или редактирования провайдера найдите блок **Кастомные промпты**.
1. Нажмите **Добавить промпт** и введите текст инструкции в текстовое поле.
1. При необходимости добавьте ещё промпты (до 20 элементов).
1. Чтобы удалить промпт, нажмите **Удалить** рядом с соответствующим полем.
1. Нажмите **Сохранить**.

**Ограничения:**

- Не более 20 кастомных промптов на провайдера.
- Длина каждого промпта — до 4096 символов.

**Примеры использования:** задание роли («Отвечай кратко и по делу»), ограничений («Не предлагай изменения в production без подтверждения»), формата («Используй списки и заголовки»).
...
1. **Формат ответа по умолчанию**: Агент возвращает краткий ответ (счётчики, короткие списки, выводы), а не сырой JSON и не полный вывод инструментов. Если нужны сырые данные или полный вывод, укажите это в запросе явно — например: «выведи JSON», «raw data», «полный вывод», «покажи полный результат инструмента».
