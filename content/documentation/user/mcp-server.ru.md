...
MCP (Model Context Protocol) сервер — это компонент Deckhouse Development Platform, который предоставляет инструменты для взаимодействия с платформой через протокол MCP. MCP-сервер позволяет внешним AI клиентам (таким как LM Studio, Claude Desktop и другие) получать доступ к данным платформы и выполнять операции через стандартизированный интерфейс.

MCP-сервер реализует протокол JSON-RPC 2.0 и предоставляет набор инструментов (tools) для работы с ресурсами платформы и получения информации из документации.

## Доступные инструменты

### 1. get_resource_entities
...
|-------------------------|--------|----------------------|---------------------------------------------------|
| `resource_name_or_slug` | строка | Имя или slug ресурса | `Сервисы`, `services`, `Deckhouse Code: Repositories` |
...
LM Studio — это популярное приложение для работы с локальными языковыми моделями, которое поддерживает протокол MCP. Для подключения MCP-сервера Deckhouse Development Platform к LM Studio выполните следующие шаги:

#### Шаг 1: Получение URL и токена доступа

1. Войдите в Deckhouse Development Platform
2. Получите ваш API токен в разделе настроек профиля
3. Запишите URL вашей платформы (например: `https://ddp.example.com`)

#### Шаг 2: Настройка в LM Studio

1. Откройте LM Studio
2. Перейдите в настройки (Settings)
3. Найдите раздел **MCP Servers** или **Model Context Protocol**
4. Нажмите **Add Server** или **Добавить сервер**
...
2. LM Studio попытается подключиться к серверу
3. Если подключение успешно, вы увидите список доступных инструментов:
...
- Модель сможет вызывать `get_resource_entities` для получения данных о ресурсах
- Модель сможет использовать `get_external_data` для выполнения запросов к внешним сервисам (GitLab, SonarQube, Kubernetes и др.)
- Все вызовы будут выполняться с вашими правами доступа

### Подключение в других MCP клиентах

MCP-сервер Deckhouse Development Platform совместим с любыми клиентами, поддерживающими протокол MCP через JSON-RPC 2.0. Общие шаги подключения:
...
MCP-сервер требует аутентификации для всех запросов:

- Используйте API токен из вашего профиля
- Токен передается через заголовок `X-API-TOKEN: TOKEN`
- Права доступа соответствуют правам вашего пользователя в платформе

### Права доступа

- Инструменты работают с теми же правами, что и ваш пользователь
...
### Проблема: Инструмент возвращает ошибку доступа
...
### Проблема: Данные не возвращаются
...
MCP (Model Context Protocol) — это открытый протокол для взаимодействия AI моделей с внешними системами. Подробнее о протоколе можно узнать на [официальном сайте MCP](https://modelcontextprotocol.io/).
