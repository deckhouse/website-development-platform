...
MCP (Model Context Protocol) сервер — это механизм Deckhouse Development Platform, который предоставляет инструменты для взаимодействия с платформой через протокол MCP. MCP-сервер позволяет внешним AI клиентам (таким как LM Studio, Claude Desktop и другие) получать доступ к данным платформы и выполнять операции через стандартизированный интерфейс.

MCP-сервер реализует протокол JSON-RPC 2.0 и предоставляет набор инструментов (tools) для работы с ресурсами платформы.
