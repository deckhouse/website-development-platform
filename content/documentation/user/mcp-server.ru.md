...
MCP-сервер — компонент Deckhouse Development Platform (DDP), который реализует протокол MCP (Model Context Protocol) и обеспечивает взаимодействие внешних AI-клиентов (таких как LM Studio, Claude Desktop и других) с платформой.
...
{{< alert level="info" >}}
Помимо встроенных инструментов платформы, MCP-сервер может вызывать инструменты из [MCP-коллекций](../mcp-management/#mcp-коллекции), к которым у пользователя есть доступ. Настройка MCP-серверов, пользовательских MCP-инструментов и коллекций выполняется в разделе [«Управление MCP»](../mcp-management/).
{{< /alert >}}

## Доступные инструменты

Ниже перечислены **встроенные** инструменты платформы. Инструменты типов `external` и `custom` доступны после [синхронизации каталога](../mcp-management/#mcp-серверы) и [добавления в MCP-коллекцию](../mcp-management/#mcp-коллекции).
...
1. Получение параметров:
   ...
   - Запишите URL платформы (например, `https://ddp.example.com`).
...
- Проверьте логи платформы для получения подробной информации об ошибке.
