...
В DDP (портал) существуют две встроенные сущности — команды и пользователи. Основным источником данных для авторизации является Dex, установленный в Deckhouse Platform.

Пользователи обычно появляются при первой авторизации через Dex; администратор также может создать учётную запись заранее. Создание команд вручную в портале не поддерживается.
...
При первой авторизации пользователя в портал для него создаётся внутренняя учётная запись. Команды пользователя синхронизируются при первой и каждой последующей авторизации пользователя.
...
Время последней активности пользователя в портале. Отображается в таблице пользователей в разделе «Администрирование» → «Пользователи». Обновляется при авторизации, либо при автоматической ротации JWT-токена пользователя (интервал ротации зависит от конфигурации Dex и по умолчанию равен 10 минутам).

{{< alert level="info" >}}
Время последней активности не обновляется при использовании API-токена, выписанного пользователем, либо при каких-либо действиях пользователя в портале в интервале между автоматическими ротациями JWT токена.
{{< /alert >}}

## Сервисные пользователи

Сервисный пользователь — учётная запись портала для автоматизации и интеграций (скрипты, технические сценарии).
